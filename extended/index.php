...
require_once 'shared/php/CrawlerDetect.php';

$crawlerDetect = new CrawlerDetect();
if ($crawlerDetect->isCrawler() == true) {
    // Generate your SEO-friendly html page here
    exit;
}
...
/*
 * // Minify JS files if need be
  $minJsFilePath = ROOT_DIR . 'cache' . DIRECTORY_SEPARATOR . 'min.js';
  $minJsFileUrl = ROOT_URL . 'cache/min.js';

  if (file_exists($minJsFilePath) == true) {
  require_once 'shared/php/Minifier.php';
  $js = '';
  foreach ($javascriptFiles as $f) {
  $f = str_replace(ROOT_URL, ROOT_DIR, $f);
  $js .= ';' . file_get_contents($f);
  }
  $minifiedCode = Minifier::minify($js, array('flaggedComments' => false));
  file_put_contents($minJsFilePath, $minifiedCode);
  }
 * <script src="<?php echo $minJsFileUrl ?>"></script>
 * 
 */
?>
<!DOCTYPE html>
...
<script>
    ...
    console.log('Current action: <?php echo $controller; ?>.<?php echo $method; ?>');
